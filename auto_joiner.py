...
class Meeting:
    # Main class that keep tracks of time and id.
...
class Channel:
    # Class that lists all the sub channels in a channel.
    def __init__(self, name, meetings, blacklisted=False):
        self.name = name
        self.meetings = meetings
        self.blacklisted = blacklisted

    # Prints the channels that are blacklisted and not blacklisted sepreately when the calss object is called.
    def __str__(self):
        return self.name + " [BLACKLISTED]" if self.blacklisted else self.name

    #Uses the Selenium.find_element_by_css to find and list all the channel names in the List mode of msteams Web app.
    def get_elem(self, parent):
        try:
            channel_elem = parent.find_element_by_css_selector(f"ul>ng-include>li[data-tid*='channel-{self.name}-li']") #String formatting is used as an argument
...
class Team:
    ...
    # Prints out all the channels when the object is called
    def __str__(self):
        channel_string = '\n\t'.join([str(channel) for channel in self.channels])

        return f"{self.name}\n\t{channel_string}" #String formatting is used

    # Expands channels by clicking on them so the subchannels can be read by scc selector
    def expand_channels(self):
        # note This gives the whole block which contains more information to be filterd out
        try:
            elem = self.elem.find_element_by_css_selector("div.channels")
             
        except exceptions.NoSuchElementException:
            try:
                self.elem.click() #click all the channel names
                elem = self.elem.find_element_by_css_selector("div.channels") #finds the sub channels
            except exceptions.NoSuchElementException:
                print("NoSuchElementException Occured")
                return None # If this is thrown the list is empty i.e No channels are there in the list or the view is incorrect.
        return elem
        

    #used to call the expand channels and to make 
    def init_channels(self):
        channels_elem = self.expand_channels()
        #further filters the chanel elements to find only the sub channel elements
        channel_elems = channels_elem.find_elements_by_css_selector("ul>ng-include>li")
        
        #print("1",channel_elems)

        #creates a lit of all channel name
        channel_names = [channel_elem.get_attribute("data-tid") for channel_elem in channel_elems]
        
       # print("2",channel_names)
    ...
    def update_meetings(self):
        ...
        #checks if there are any active calls in the channel 
        for channel in self.channels:
            if channel.blacklisted:
                continue

            #Gets the source of the channel
            channel_elem = channel.get_elem(channels)
            
            #if any active or scheduled calls are there in channel it clicks them
            try:
            ...
            #waits 60 seconds if the join meeting button is not there
            if wait_until_found("button[ng-click='ctrl.joinCall()']", 60) is None:
                continue

            #finds the source of the join meeting channel
...
def check_if_found(sel, timeout):
...
def findPoll():
    ...
    wait_until_found(".icons-start-conversation", 30)
    
    browser.find_element_by_css_selector(".icons-start-conversation").click()
...
def main():
    ...
    while 1:
        ...
        if join_newest_meeting(teams):
            ...
            print("Enterd the loop to check for new meetings after 30 min")
            ...
            if(join_newest_meeting(teams) or active_meeting == None):
                print("none")
            else:
                print("No new meeting found checking for poll")
                print("putting function to sleep")
                time.sleep(10)
                enlarge = browser.find_element_by_css_selector(".call-status")
                enlarge.click()
                findPoll()
                i=1
